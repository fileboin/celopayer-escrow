...
function PaymentApp() {
  ...
  const [mode, setMode] = useState<'escrow' | 'instant'>(() => {
    const m = searchParams?.get('mode')
    return (m === 'instant' || m === 'escrow') ? m : 'escrow'
  })
  ...
  const [selectedTokenIndex, setSelectedTokenIndex] = useState(() => {
    ...
    if (tokenParam) {
      const idx = TOKENS.findIndex(t => t.symbol === tokenParam)
      return idx >= 0 ? idx : 0
    }
    return 0
  })
  ...
  const generateInvoiceUrl = () => {
    ...
    params.append('token', TOKENS[selectedTokenIndex].symbol)
    ...
    try {
      ...
      if (!isConnected) {
        ...
        if (effectiveMode === 'instant') {
          ...
          setStatusMsg(t.waitTrans + ' (Approve: ' + approveTx.slice(0, 10) + '...)')
        ...
        setStatusMsg(t.waitTrans + ' (Hash: ' + txHash.slice(0, 10) + '...)')
  ...
  const getShareUrl = () => {
    ...
    params.set('token', TOKENS[selectedTokenIndex].symbol)
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors flex flex-col items-center justify-center p-4 sm:p-8">
      <div className="w-full max-w-md">
                ...
                <div className="absolute right-0 mt-2 w-32 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 z-50">
                  {(['en', 'sr', 'es', 'fr'] as Language[]).map(l => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 first:rounded-t-xl last:rounded-b-xl transition-colors ${lang === l ? 'bg-celo-green text-white' : 'text-gray-700 dark:text-gray-300'}`}
                    >
            ...
            <div className="relative">
              ...
              {showThemeMenu && (
                <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 z-50">
                  <button onClick={() => { setTheme('celo-yellow'); setShowThemeMenu(false) }} className={`w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 first:rounded-t-xl transition-colors ${theme === 'celo-yellow' ? 'bg-celo-green text-white' : 'text-gray-700 dark:text-gray-300'}`}>🌞 Celo Yellow</button>
                  <button onClick={() => { setTheme('celo-modern'); setShowThemeMenu(false) }} className={`w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${theme === 'celo-modern' ? 'bg-celo-green text-white' : 'text-gray-700 dark:text-gray-300'}`}>🌙 Modern Dark</button>
                  <button onClick={() => { setTheme('light'); setShowThemeMenu(false) }} className={`w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${theme === 'light' ? 'bg-celo-green text-white' : 'text-gray-700 dark:text-gray-300'}`}>☀️ Light</button>
                  <button onClick={() => { setTheme('dark'); setShowThemeMenu(false) }} className={`w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 last:rounded-b-xl transition-colors ${theme === 'dark' ? 'bg-celo-green text-white' : 'text-gray-700 dark:text-gray-300'}`}>🌙 Dark</button>
                </div>
              )}
            </div>
            ...
            {!isConnected ? (
              ...
              <div className="flex items-center gap-2 bg-green-100 dark:bg-green-900/20 border border-green-300 dark:border-green-700 px-3 py-2 rounded-full">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setShowConnectMenu(!showConnectMenu)} className="p-1 rounded-full hover:bg-green-200 dark:hover:bg-green-800 transition-colors" title="Switch Wallet">
                    <Wallet size={14} className="text-green-600 dark:text-green-400" />
                  </button>
                  <button onClick={() => disconnect()} className="p-1 rounded-full hover:bg-red-200 dark:hover:bg-red-800 transition-colors" title={t.disconnect}>
        ...
        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-full mb-6">
          <button onClick={() => setFlow('send')} className={`flex-1 py-3 text-sm font-bold rounded-full flex justify-center items-center gap-2 transition-all duration-300 ${flow === 'send' ? 'bg-[#2AAB66] text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white'}`}>
            <Send size={18} />
            {t.sendPayment}
          </button>
          <button onClick={() => setFlow('request')} className={`flex-1 py-3 text-sm font-bold rounded-full flex justify-center items-center gap-2 transition-all duration-300 ${flow === 'request' ? 'bg-[#F6C644] text-black shadow-md' : 'text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white'}`}>
            <QrCode size={18} />
            {t.requestPayment}
          </button>
          <button onClick={() => setFlow('scheduled')} className={`flex-1 py-3 text-sm font-bold rounded-full flex justify-center items-center gap-2 transition-all duration-300 ${flow === 'scheduled' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white'}`}>
            <Wallet size={18} />
            {t.scheduled}
          </button>
        </div>
        ...
        {flow === 'send' && (
          <>
            <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-full mb-4">
              <button onClick={() => setMode('escrow')} className={`flex-1 py-3 text-sm font-bold rounded-full flex justify-center items-center gap-2 transition-all duration-300 ${mode === 'escrow' ? 'bg-black dark:bg-white text-white dark:text-black shadow-md' : 'text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white'}`}>
                <ShieldAlert size={18} />
                {t.escrow}
              </button>
              <button onClick={() => setMode('instant')} className={`flex-1 py-3 text-sm font-bold rounded-full flex justify-center items-center gap-2 transition-all duration-300 ${mode === 'instant' ? 'bg-black dark:bg-white text-white dark:text-black shadow-md' : 'text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white'}`}>
                <Zap size={18} />
                {t.instant}
              </button>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-[2rem] shadow-xl shadow-gray-200/40 dark:shadow-none border border-gray-100 dark:border-gray-700 p-6 sm:p-8 mb-6 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-2">
              {TOKENS.map((token, idx) => (
                <button
                  ...
                  className={`py-2 text-xs font-bold rounded-xl border transition-all ${selectedTokenIndex === idx ? 'bg-celo-green text-white border-celo-green shadow-md' : 'bg-gray-50 dark:bg-gray-900 text-gray-500 border-gray-200 dark:border-gray-700 hover:border-celo-green'}`}
                >
                  {token.symbol}
                </button>
              ))}
          ...
          <div className="mb-5">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowContacts(true)} className="text-[10px] font-bold text-blue-600 hover:underline flex items-center gap-1">
                  <Search size={10} /> {t.contacts}
                </button>
                {flow === 'request' && isConnected && (
                  <button onClick={() => setRecipient(address || '')} className="text-[10px] font-bold text-celo-green hover:underline">
                    {t.useMyAddress}
                  </button>
                )}
              </div>
            ...
            <input
              type="text"
              value={recipient}
              onChange={(e) => { setRecipient(e.target.value); setErrorMsg('') }}
          ...
          {flow !== 'scheduled' && (
            <div className="relative mb-8">
              <button
                ...
                className="group relative w-full p-6 text-white font-black text-xl rounded-3xl transition-all duration-300 shadow-2xl flex items-center justify-center gap-4 transform hover:scale-105 border-4 bg-gradient-to-r from-celo-green to-green-600 hover:from-green-600 hover:to-green-700 border-green-700 shadow-green-500/50"
              >
                <div className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-green-600 to-green-700"></div>
                {isPending ? (
                  <>
              ...
              <div className="absolute inset-0 rounded-3xl blur-xl opacity-50 bg-green-500 -z-10"></div>
            </div>
          )}
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 dark:border-gray-700 flex flex-col items-center">
            ...
            <div className="p-3 bg-white border border-gray-200 shadow-sm rounded-2xl mb-4 transition-colors">
              <QRCodeSVG
                value={getShareUrl() || `${typeof window !== 'undefined' ? window.location.origin : ''}/?to=${recipient || ''}&amount=${amount || '0'}&mode=${mode}&token=${TOKENS[selectedTokenIndex].symbol}`}
                size={140}
                fgColor="#171717"
              />
            </div>
            ...
            <div className="mt-6 w-full">
              ...
              <div className="grid grid-cols-4 gap-3">
                <button onClick={() => shareVia('whatsapp')} className="flex flex-col items-center gap-2 p-3 bg-green-50 dark:bg-green-900/10 rounded-2xl hover:scale-105 transition-transform" title="WhatsApp">
                  <MessageCircle size={24} className="text-[#25D366]" />
                  <span className="text-[10px] font-bold text-gray-500">WhatsApp</span>
                </button>
                <button onClick={() => shareVia('telegram')} className="flex flex-col items-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/10 rounded-2xl hover:scale-105 transition-transform" title="Telegram">
                  <Send size={24} className="text-[#0088cc]" />
                  <span className="text-[10px] font-bold text-gray-500">Telegram</span>
                </button>
                <button onClick={() => shareVia('viber')} className="flex flex-col items-center gap-2 p-3 bg-purple-50 dark:bg-purple-900/10 rounded-2xl hover:scale-105 transition-transform" title="Viber">
                  <MessageCircle size={24} className="text-[#7360f2]" />
                  <span className="text-[10px] font-bold text-gray-500">Viber</span>
                </button>
                <button onClick={() => shareVia('mail')} className="flex flex-col items-center gap-2 p-3 bg-gray-100 dark:bg-gray-800 rounded-2xl hover:scale-105 transition-transform" title="Email">
          ...
          {tasks.length > 0 && (
            <div className="mt-8">
              ...
              <div className="space-y-3">
                {tasks.map(task => (
                  <div key={task.id} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex justify-between items-center">
                    <div>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => payScheduled(task)} className="px-3 py-1.5 bg-gray-900 dark:bg-white text-white dark:text-black text-[10px] font-bold rounded-lg hover:opacity-80">
                        {t.payWith}
                      </button>
                      <button onClick={() => deleteTasks(task.id)} className="px-3 py-1.5 bg-red-50 dark:bg-red-900/20 text-red-600 text-[10px] font-bold rounded-lg hover:bg-red-100">
                        ✕
                      </button>
                    </div>
                  </div>
        ...
        <div className="mt-6 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-2xl p-6 border border-purple-200 dark:border-purple-800">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => { setRecipient(address || ''); setAmount('10'); setMode('instant'); setFlow('send') }} className="p-3 bg-white dark:bg-gray-800 rounded-xl border border-purple-200 dark:border-purple-700 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
            ...
            <button onClick={() => { setRecipient(address || ''); setAmount('25'); setMode('escrow'); setFlow('request') }} className="p-3 bg-white dark:bg-gray-800 rounded-xl border border-purple-200 dark:border-purple-700 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
            ...
            <button onClick={() => { setFlow('scheduled'); setFrequency('week') }} className="p-3 bg-white dark:bg-gray-800 rounded-xl border border-purple-200 dark:border-purple-700 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
            ...
            <button onClick={() => setShowHistory(true)} className="p-3 bg-white dark:bg-gray-800 rounded-xl border border-purple-200 dark:border-purple-700 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
        ...
        <div className="mt-6 bg-gradient-to-r from-blue-50 to-green-50 dark:from-blue-900/20 dark:to-green-900/20 rounded-2xl p-6 border border-blue-200 dark:border-blue-800">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-1 rounded-full font-semibold">NEW</span>
          </div>
          <BluetoothPayment
            recipient={recipient}
            amount={amount}
              ...
              <div className="p-6">
                <div className="flex gap-2 mb-6">
                  <div className="flex-1 space-y-2">
                    <input placeholder={t.name} value={newContact.name} onChange={(e) => setNewContact({ ...newContact, name: e.target.value })} className="w-full p-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none" />
                    <input placeholder={t.address} value={newContact.address} onChange={(e) => setNewContact({ ...newContact, address: e.target.value })} className="w-full p-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none font-mono" />
                  </div>
                  <button onClick={addContactToList} className="bg-blue-600 text-white px-4 rounded-xl font-bold text-sm">{t.save}</button>
                </div>
                <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                  {contacts.map(c => (
                    <div key={c.address} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-100 dark:border-gray-700">
                      ...
                      <button onClick={() => deleteContact(c.address)} className="text-red-500 p-2"><X size={14} /></button>
                    </div>
                  ))}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-700">
              <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                ...
                <button onClick={() => setShowHistory(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full"><X size={20} /></button>
              </div>
              <div className="p-6 max-h-[70vh] overflow-y-auto space-y-3">
                {history.map(tx => (
                  <div key={tx.hash} className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-700">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <button onClick={() => downloadReceipt(tx)} className="p-2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg shadow-sm hover:scale-105 transition-transform" title={t.downloadReceipt}>
                        <FileText size={14} className="text-celo-green" />
                      </button>
                    </div>
                    <p className="text-[10px] text-gray-500 font-mono break-all line-clamp-1">{tx.to}</p>
                    <a href={`https://celoscan.io/tx/${tx.hash}`} target="_blank" rel="noreferrer" className="mt-2 block text-[9px] font-bold text-celo-green hover:underline">
                      View on Explorer →
                    </a>
                  </div>
                ))}
